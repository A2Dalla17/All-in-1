...
func TestKubernetesProvider_FetchFile(t *testing.T) {
	...
	err = os.WriteFile(secretPath, []byte("my-secret-value"), 0o600)
	if err != nil {
...
func TestKubernetesProvider_FetchDirectory(t *testing.T) {
	...
	err = os.MkdirAll(secretDir, 0o700)
	if err != nil {
		t.Fatalf("failed to create secret dir: %v", err)
	}

	err = os.WriteFile(filepath.Join(secretDir, "username"), []byte("admin"), 0o600)
	if err != nil {
		t.Fatalf("failed to write username: %v", err)
	}
	err = os.WriteFile(filepath.Join(secretDir, "password"), []byte("secret123"), 0o600)
	if err != nil {
