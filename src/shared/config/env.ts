...
export const env = {
  ...
  /**
   * Where "book in your browser" goes.
   *
   * The native apps are not published yet, so the store buttons on this site
   * render as "coming soon". This is the one route to a car that works today,
   * and it has to keep working — a website that can only offer two unavailable
   * downloads is a website that turns customers away.
   *
   * Empty by default rather than pointing at a guess: a button leading to a
   * dead URL is worse than one that is honestly absent.
   */
  appUrl: readString(import.meta.env['VITE_APP_URL'], ''),
